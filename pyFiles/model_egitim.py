...
class ModelEgitim:
    ...
    def fit(self, cv=10):
        ...
        svr_params = {
            ...
            "Regressor__epsilon": (1e-03, 1e-04, 0.01, 0.1),
            "Regressor__coef0": (
                1.0,
                0.0,
                0.5,
        ...
        mlp_params = {
            ...
            "Regressor__beta_1": (0.85, 0.90, 0.95, 0.99),
            "Regressor__beta_2": (0.85, 0.90, 0.95, 0.99),
        ...
        mdlIndex = [
            idx
            for idx, modname in enumerate(self.allModelNames)
        ...
        for i in range(len(regressors)):
            mod = mdls[i]
            pipe = Pipeline(
                [("preprocess", preprocessor), ("Regressor", regressors[i])]
            )
            startTime = time.time()
            ...
            esRMSE = mean_squared_error(
                y_pred=results.predict(self.X), y_true=self.y, squared=True
            )
            esMAE = mean_absolute_error(y_pred=results.predict(self.X), y_true=self.y)
            esMAPE = mean_absolute_percentage_error(
                y_pred=results.predict(self.X), y_true=self.y
            )
            esR2 = r2_score(y_pred=results.predict(self.X), y_true=self.y)
            print("".ljust(60, "*"))
            print(f"Veri satır sayısı {self.X.shape[0]}")
            print(f"Model : {mod}")
            print(f"Eğitim süresi : {timeSpent} saniye.")
            print(
                f"Eğitim skoru - RMSE : {math.sqrt(esRMSE):#.3f}, MAE : {esMAE:#.3f}, R2 : {esR2:#.3f}, MAPE : {esMAPE:#.3f}"
            )
            print(f"CV skoru - RMSE : {math.sqrt(np.nanmean(cvSkoru)):#.3f}\n\n")
            joblib.dump(
                results.best_estimator_, "results/" + mod + "_mod.pkl", compress=9
            )
