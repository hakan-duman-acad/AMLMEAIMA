...
t_test <- function(data) {
  ...
  if (shap2$p.value>0.05) {
    ...
    tt <- oneway_test(score ~ type, data = data, 
                      distribution = approximate(nresample = 10000)) 
    tst <- "perm"
  }
  return(list(normby = shap1, norm=shap2, var=lev, test = tt, tye=tst))       
}
  modData <- data  %>% filter(metric == smetric, model == mod ) 
  result <- t_test(modData)
  lev <- sprintf("Levene Testi[F(%s) = %s, p = %.3f] homojenite varsayımı %s.", 
                    paste(result$var$Df,collapse = ", "),
                    fm(result$var$`F value`[1]),
                    result$var$`Pr(>F)`[1],
                 ifelse(result$var$`Pr(>F)`[1]>0.05,"sağlanıyor","sağlanmıyor")
                 ) 
  shap <- sprintf("Shapiro Wilk Testi [W = %s, p = %.3f] normallik koşulu %s.", 
  ...
  if (result$tye == "perm") {
    ...
    sta <- fm(result$test$statistic)
      ...
      test <- sprintf("%s [t(%s) = %s, p = %.3f] aşırı veya az uyum problemi %s.",
